 ...
 * Pie Card Deck
 */ 
public class PieCardDeck : CardDeckInterface
{
    ...
    protected override void ReArrangeCards()
    {
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            Vector3 pos = new Vector3(Mathf.Sin(currentAngle), Mathf.Cos(currentAngle), i * -.001f) * -radius;
            if (cards[i] != null)
    ...
    protected override void IndexUpdate()
    {
        ...
        if (offsetInDegree != target)
        {
            ...
            if (Mathf.Abs(Mathf.DeltaAngle(offsetInDegree, target)) < 0.01f)
            {
